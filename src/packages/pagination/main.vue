<template>
    <div class="zrx-pagination">
        <div :class="[position, vmAlign]" v-for="position in ['left', 'right']" :style="computeWrapStyle(position)">
...
export default {
    ...
    computed: {
        ...
        vmAlign() {
            return ['small', 'mini'].includes(this.pagerSize) ? this.align : ''
        },
        vmLayout() {
            return { small: 'prev, pager, next', mini: 'prev, next' }[this.pagerSize] ?? this.layout
        }
    },
    methods: {
    ...
    mounted: function () {
        const layoutComponent = this.vmLayout.split(',').map(n => n.trim())
