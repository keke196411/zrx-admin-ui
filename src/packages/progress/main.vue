...
export default {
    ...
    methods: {
        setStep(duration) {
            const expectation = 100 / (duration * this.options.granularity) // 每次预计增加的百分比值
        ...
        start(duration) {
            ...
            const interval = (1 / this.options.granularity) * 1000 // 每次更新进度条的间隔
            this.state.cut = this.setStep(duration) // 在大计算量下，该值期望为0.5
