...
export default {
        ...
        tabs: {
            handler(newv, oldv) {
                if (oldv && newv.length < oldv.length) {
                    this.init(oldv.findIndex(d => d[this.setting.key] == this.value))
                } else {
                    this.init()
                }
            },
    ...
    methods: {
        async init(oldIndex) {
            ...
            this.setShift()
            this.setCurrentSelected(this.value, oldIndex)
            if (this.pattern == 'toggle') {
                this.toggler = this.value
            }
        },
        // 如果tabs变动后，原来选中的tab不见了，先从前往后再从后往前寻找新的选中tab
        findNearestIndex(originIndex) {
            const loop = (index, dir = -1) => {
                if (dir == -1) {
                    if (index + dir >= 0) {
                        index += dir
                        if (index in this.listData) return index
                        return loop(index, dir)
                    } else {
                        dir = 1
                        return loop(originIndex, dir)
                    }
                } else {
                    if (originIndex in this.listData) return originIndex
                    if (index + dir <= this.listData.length - 1) {
                        index += dir
                        if (index in this.listData) return index
                        return loop(index, dir)
                    } else {
            ...
            return loop(originIndex)
        },
        setCurrentSelected(v, originIndex) {
            let targetIndex = this.listData.findIndex(item => item[this.setting.key] === v)
            if (targetIndex == -1 && originIndex !== undefined) {
                targetIndex = this.findNearestIndex(originIndex)
            }
            if (targetIndex != -1) {
        ...
        extendList() {
            this.listData = this.tabs.map((item, index) => {
                return { ...item, shift: 0, el: `tab_${index}`, width: 0, value: item[this.setting.key] }
            })
