                    ...
                    <span class="icon-wrap" :style="`--indent: ${indents[0]}px;`">
                        <slot name="icon" :icon="main[props.icon]">
                            <template v-if="main.iconType">
                                ...
                                <span class="image-svg" v-else-if="main.iconType == 'imageSvg'">
                                    <img :src="main[props.icon]" @error="onerror(main)" />
                                </span>
                            ...
                            <span class="icon-wrap" :style="`--indent: ${indents[1]}px;`">
                                <slot name="icon" :icon="sub[props.icon]">
                                    <template v-if="sub.iconType">
                                        ...
                                        <span class="image-svg" v-else-if="main.iconType == 'imageSvg'">
                                            <img :src="sub[props.icon]" @error="onerror(sub)" />
                                        </span>
...
export default {
    ...
    methods: {
                ...
                for (const item of list) {
                    ...
                    if (item[this.props.icon]) {
                        if (item[this.props.icon].match(/^https?:\/\/\w+/i)) {
                            if (/\w+.svg[^.]*$/i.test(item[this.props.icon])) {
                                item.iconType = 'imageSvg'
                            } else {
                                item.iconType = 'image'
                            }
                        } else {
        ...
        updateCache(props) {
            if (this.cacheObj.cache.length && this.cacheObj.index != this.cacheObj.cache.length - 1) {
                this.cacheObj.cache.splice(this.cacheObj.index + 1)
            }
            this.cacheObj.cache.push(props)
            this.cacheObj.cache.length > 20 && this.cacheObj.cache.shift()
            this.cacheObj.index = this.cacheObj.cache.length - 1
        },
        toggleDroppedAndSelected(navObj) {
            if (navObj[this.props.children]?.length) {
                ...
                if (!navObj.selected) {
                    ...
                    const record = { target: navObj, current: this.current }
                    if (this.accordian) {
                        const root = this.findParent(navObj)
                        this.resetStatus('dropped', root)
                        Object.assign(record, { root })
                    }
        ...
        setCurrent(navObj) {
            ...
            if (typeof navObj == 'string') {
                navObj = this.findData('path', navObj)
                if (!navObj) return
            }
            if (navObj[this.props.children]?.length) {
                // 可下拉不可选中
        ...
        wander(dir) {
            const max = this.cacheObj.cache.length - 1
            if (dir == 'back' && this.cacheObj.index > 0) {
                this.cacheObj.index--
            } else if (dir == 'next' && this.cacheObj.index < max) {
                this.cacheObj.index++
            } else if (typeof dir == 'number' && dir < 0 && this.cacheObj.index != max + dir && max + dir >= 0) {
                this.cacheObj.index = max + dir
            } else {
                return false
            }
            const record = this.cacheObj.cache[this.cacheObj.index]
            record.target.selected = true
            this.resetStatus('selected', record.target)
            if (record.root) {
                record.root != record.target && (record.root.dropped = true)
                this.accordian && this.resetStatus('dropped', record.root)
            }
