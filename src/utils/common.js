...
export function setElTreeNodeExpanded(nodeObj, expanded = true) {
    if (nodeObj?.parent) {
        nodeObj.parent.expanded = expanded
        setElTreeNodeExpanded(nodeObj.parent, expanded)
    }
}

export function setElTreeExpanded(treeObj, keys) {
    if (!treeObj) return
    const nodes = []
    keys.forEach(key => {
        const nodeObj = treeObj.getNode(key)
        if (nodeObj) {
            setElTreeNodeExpanded(nodeObj)
            nodes.push(nodeObj)
        }
    })
    return nodes
